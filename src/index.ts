...
async function collectLiveShadowCertification(env:Env){
 ...
 const missingCand=Number((await env.DB.prepare(`SELECT COUNT(DISTINCT p.prop_id) n FROM model_predictions p WHERE p.model_version_id=? AND p.prediction_mode='PRODUCTION' AND p.prediction_status='COMPLETE' AND p.predicted_at>=? AND UPPER(COALESCE(p.preferred_side,'NONE')) IN ('MORE','LESS') AND NOT EXISTS(SELECT 1 FROM model_predictions c WHERE c.prop_id=p.prop_id AND c.model_version_id=? AND c.prediction_mode='SHADOW' AND c.prediction_status='COMPLETE' AND c.predicted_at>=?)`).bind(prodId,start,candId,start).first<{n:number}>())?.n??0);
 const sourceExcluded=Number((await env.DB.prepare(`SELECT COUNT(DISTINCT p.prop_id) n FROM model_predictions p WHERE p.model_version_id=? AND p.prediction_mode='PRODUCTION' AND p.prediction_status='COMPLETE' AND p.predicted_at>=? AND UPPER(COALESCE(p.preferred_side,'NONE')) NOT IN ('MORE','LESS')`).bind(prodId,start).first<{n:number}>())?.n??0);
 const failures=(await env.DB.prepare(`SELECT f.live_shadow_failure_id,f.model_prediction_id,f.prop_id,f.board_date,f.failed_at,f.failure_type,f.error_message,pi.canonical_name pitcher_name FROM live_shadow_failure_ledger f JOIN props pr ON pr.prop_id=f.prop_id JOIN pitchers pi ON pi.pitcher_id=pr.pitcher_id WHERE f.live_shadow_certification_id=? AND f.failure_type='SHADOW_RUNTIME' AND COALESCE(f.error_message,'') NOT LIKE 'v14 baseline requires a directional source recommendation%' ORDER BY f.failed_at DESC LIMIT 100`).bind(cert.live_shadow_certification_id).all<Record<string,unknown>>()).results??[];
...
async function getPromotionReadiness(env:Env):Promise<Response>{
  const {readiness,certification}=await collectPromotionReadinessV92(env);return json({release:'3.8',build:'9.2.1',governance_version:'promotion-gate-v1',...readiness,certification,promotion_enabled:false,note:'Build 9.2.1 promotion gates use only certification-eligible directional evidence inside the active window. Non-directional source rows are preserved as accounted exclusions, not runtime failures.'});
}
