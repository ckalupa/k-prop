...
async function collectLiveShadowCertification(env:Env){
 ...
 // Build 9.2.5: one indexed ledger pass replaces the prior correlated per-prop scans.
...
async function getPromotionReadiness(env:Env):Promise<Response>{
  const {readiness,certification}=await collectPromotionReadinessV92(env);return json({release:'3.8',build:'9.2.5',governance_version:'promotion-gate-v1',...readiness,certification,promotion_enabled:false,note:'Build 9.2.5 promotion gates use indexed certification-eligible directional evidence inside the active window. Non-directional source rows are preserved as accounted exclusions, not runtime failures.'});
}
