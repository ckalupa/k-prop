...
async function captureV14BaselinePredictionLedger(
  ...
  if (side === "NONE") throw new Error(`v14 baseline requires a directional source recommendation for prop ${prop.prop_id}.`);
...
async function collectLiveShadowCertification(env:Env){
 ...
 const rows=(await env.DB.prepare(`SELECT p.prop_id,b.board_date,pr.result,p13.preferred_side v13_side,CASE WHEN UPPER(p13.preferred_side)='MORE' THEN COALESCE(p13.calibrated_more_probability,p13.raw_more_probability) ELSE COALESCE(p13.calibrated_less_probability,p13.raw_less_probability) END v13_probability,p14.preferred_side v14_side,CASE WHEN UPPER(p14.preferred_side)='MORE' THEN COALESCE(p14.calibrated_more_probability,p14.raw_more_probability) ELSE COALESCE(p14.calibrated_less_probability,p14.raw_less_probability) END v14_probability FROM props p JOIN boards b ON b.board_id=p.board_id JOIN model_predictions p13 ON p13.model_prediction_id=(SELECT x.model_prediction_id FROM model_predictions x WHERE x.prop_id=p.prop_id AND x.model_version_id=? AND x.prediction_mode='PRODUCTION' AND x.prediction_status='COMPLETE' AND x.predicted_at>=? ORDER BY x.model_prediction_id DESC LIMIT 1) JOIN model_predictions p14 ON p14.model_prediction_id=(SELECT x.model_prediction_id FROM model_predictions x WHERE x.prop_id=p.prop_id AND x.model_version_id=? AND x.prediction_mode='SHADOW' AND x.prediction_status='COMPLETE' AND x.predicted_at>=? ORDER BY x.model_prediction_id DESC LIMIT 1) LEFT JOIN prop_results pr ON pr.prop_id=p.prop_id AND pr.result_status<>'PENDING' ORDER BY b.board_date,p.prop_id`).bind(prodId,start,candId,start).all<Record<string,unknown>>()).results??[];
 const missingProd=Number((await env.DB.prepare(`SELECT COUNT(DISTINCT c.prop_id) n FROM model_predictions c WHERE c.model_version_id=? AND c.prediction_mode='SHADOW' AND c.prediction_status='COMPLETE' AND c.predicted_at>=? AND NOT EXISTS(SELECT 1 FROM model_predictions p WHERE p.prop_id=c.prop_id AND p.model_version_id=? AND p.prediction_mode='PRODUCTION' AND p.prediction_status='COMPLETE' AND p.predicted_at>=?)`).bind(candId,start,prodId,start).first<{n:number}>())?.n??0);
 const missingCand=Number((await env.DB.prepare(`SELECT COUNT(DISTINCT p.prop_id) n FROM model_predictions p WHERE p.model_version_id=? AND p.prediction_mode='PRODUCTION' AND p.prediction_status='COMPLETE' AND p.predicted_at>=? AND NOT EXISTS(SELECT 1 FROM model_predictions c WHERE c.prop_id=p.prop_id AND c.model_version_id=? AND c.prediction_mode='SHADOW' AND c.prediction_status='COMPLETE' AND c.predicted_at>=?)`).bind(prodId,start,candId,start).first<{n:number}>())?.n??0);
 const failures=(await env.DB.prepare(`SELECT f.live_shadow_failure_id,f.model_prediction_id,f.prop_id,f.board_date,f.failed_at,f.failure_type,f.error_message,pi.canonical_name pitcher_name FROM live_shadow_failure_ledger f JOIN props pr ON pr.prop_id=f.prop_id JOIN pitchers pi ON pi.pitcher_id=pr.pitcher_id WHERE f.live_shadow_certification_id=? ORDER BY f.failed_at DESC LIMIT 100`).bind(cert.live_shadow_certification_id).all<Record<string,unknown>>()).results??[];
